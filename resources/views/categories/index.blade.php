        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm text-gray-600" id="categories-table">
                        ...
                        <tbody class="divide-y divide-gray-100" id="table-body">
                            @forelse ($categories as $category)
                                <tr class="hover:bg-gray-50 transition category-row" data-name="{{ strtolower($category->category_name) }}">
                                    <td class="px-6 py-4 font-medium">{{ $category->category_id }}</td>
                                    <td class="px-6 py-4 font-bold text-gray-800 category-name">{{ $category->category_name }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center gap-3">
                                            ...
                                            <form method="POST" action="{{ route('categories.destroy', $category->category_id) }}" onsubmit="return confirm('Are you sure you want to delete this category?');" class="inline">
                                                @csrf
                                                @method('DELETE')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    if (noResultsRow) {
                        if (!hasResults && categoryRows.length > 0) {
