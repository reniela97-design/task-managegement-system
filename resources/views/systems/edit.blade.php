    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200 p-8">
                <form method="POST" action="{{ route('systems.update', $system->system_id) }}" class="max-w-xl">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-4">
                        <label for="system_name" class="block text-sm font-bold text-gray-700 uppercase tracking-wide mb-2">System Name</label>
                        <input type="text" name="system_name" id="system_name" value="{{ old('system_name', $system->system_name) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('systems.index') }}" class="text-gray-500 hover:text-gray-700 font-bold text-xs uppercase">
                            Cancel
                        </a>
                    </div>
                </form>
