                            ...
                            @foreach ($types as $type)
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4 text-right flex justify-end gap-2">
                                        <a href="{{ route('types.edit', $type->type_id) }}" class="text-blue-600 hover:text-blue-900 font-bold text-xs uppercase">Edit</a>
                                        <form method="POST" action="{{ route('types.destroy', $type->type_id) }}" onsubmit="return confirm('Are you sure?');">
                                            @csrf
                                            @method('DELETE')
