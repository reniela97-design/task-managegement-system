        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm text-gray-600" id="types-table">
                        ...
                        <tbody class="divide-y divide-gray-100" id="table-body">
                            @forelse ($types as $type)
                                <tr class="hover:bg-gray-50 transition type-row" data-name="{{ strtolower($type->type_name) }}">
                                    <td class="px-6 py-4 font-medium">{{ $type->type_id }}</td>
                                    <td class="px-6 py-4 font-bold text-gray-800 type-name">{{ $type->type_name }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex justify-end items-center gap-3">
                                            ...
                                            <form method="POST" action="{{ route('types.destroy', $type->type_id) }}" onsubmit="return confirm('Are you sure you want to delete this type?');" class="inline">
                                                @csrf
                                                @method('DELETE')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    if (noResultsRow) {
                        if (!hasResults && typeRows.length > 0) {
