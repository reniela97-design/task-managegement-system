...
<tr class="hover:bg-{{ $hoverColor }}-100/40 transition cursor-pointer group" @click="modalData = {{ json_encode($taskData) }}; showModal = true;">
    ...
    <td class="px-6 py-4">
        @php
            ...
            // Dynamic Text Matching (Ignores Database ID completely)
            if (str_contains($statusLower, 'complete') || str_contains($statusLower, 'done')) {
                $badgeColor = 'bg-emerald-100 text-emerald-700 border-emerald-200';
                $iconPath = 'M5 13l4 4L19 7';
            } elseif (str_contains($statusLower, 'progress') || str_contains($statusLower, 'review') || str_contains($statusLower, 'active')) {
                ...
                $isSpinning = true;
            } elseif (str_contains($statusLower, 'cancel') || str_contains($statusLower, 'abort')) {
                $badgeColor = 'bg-red-100 text-red-700 border-red-200';
                $iconPath = 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636';
            } elseif (str_contains($statusLower, 'hold') || str_contains($statusLower, 'pause')) {
